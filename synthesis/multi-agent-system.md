...
**1) 정적 MAS에서 자기진화형 MAS로.** 에이전트의 코드·프롬프트·전략을 런타임에 수정하는 메타 수준의 자기 개선 루프가 핵심 설계 원칙으로 부상하고 있다. Self-Driving Portfolio의 메타 에이전트가 가장 선명한 사례이며, CliffSearch의 이론-코드 공진화와 Agentic FL의 적응적 클라이언트 선택도 동일한 패러다임에 속한다. 에이전트 시스템의 적응성을 외부 설계자가 아닌 시스템 자신이 담보하는 방향으로의 전환이다.

**2) 효율성에서 안전·거버넌스로 무게 중심 이동.** 초기 MAS 연구가 조율의 효율성에 집중했다면, 최근 연구는 기만(Deception in Among Us), 사회적 편향(Social Dynamics), 인간 귀인 오류(Biased Error Attribution) 등 거버넌스 문제로 초점을 옮기고 있다. 이는 MAS가 실제 배포 단계에 진입하면서 안전성 검증의 필요성이 급증한 것과 맥락을 같이한다.

**3) 인간-MAS 인터페이스의 인지적 설계.** 단순한 대시보드 제공을 넘어, 인간 감독자의 인지 편향(Biased Error Attribution)과 에이전트 집단의 사회적 역학(Social Dynamics)을 동시에 고려하는 인터페이스 설계가 과제로 떠오르고 있다. Self-Driving Portfolio가 제시한 '실행자에서 감독자로'의 역할 전환은 이 방향의 구체적 실현 사례다.