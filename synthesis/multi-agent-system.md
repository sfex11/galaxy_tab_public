...
**1) 정적 MAS에서 자기진화형 MAS로.** 에이전트의 코드·프롬프트·전략을 런타임에 수정하는 메타 수준의 자기 개선 루프가 핵심 설계 원칙으로 부상하고 있다. Self-Driving Portfolio의 메타 에이전트가 가장 선명한 사례이며, CliffSearch의 이론-코드 공진화, Agentic FL의 동적 오케스트레이션도 동일한 패러다임에 속한다. 에이전트 시스템이 배포 후에도 환경 피드백을 통해 스스로를 진화시키는 구조가 표준화될 가능성이 높다.

**2) 성능 중심에서 안전·거버넌스 인식 설계로.** 초기 MAS 연구가 효율성과 정확도에 집중했다면, 최신 연구는 시스템의 사회적 역학이 초래하는 취약성에 주목한다. Social Dynamics의 편향 실증, Deception in Among Us의 기만 능력 입증, Biased Error Attribution의 인간 감독 한계 분석은 모두 MAS 배포 전 안전 평가 프레임워크의 필요성을 강하게 시사한다. 특히 에이전트 간 상호작용 자체가 간접적 공격 벡터로 기능할 수 있다는 발견은 새로운 보안 패러다임을 요구한다.

**3) 동종 에이전트에서 이종 역할 분화로.** Paper Circle의 검색-평가-합성 분업, Self-Driving Portfolio의 전망-구성-비평-감독 계층, Meanings & Measurements의 의미-측정 분리 등 에이전트 간 인지적 역할 분화가 세밀해지고 있다. 이는 단순한 작업 분할을 넘어, 인간 조직의 기능적 분화를 모방하는 구조적 성숙도의 증거다.

종합하면, 2026년 초의 MAS 연구는 **"얼마나 많은 에이전트를 배치할 것인가"**에서 **"에이전트 집단의 사회적 동학을 어떻게 설계하고 검증할 것인가"**로 무게 중심이 이동하는 과도기에 있다. 자기진화 능력과 구조적 취약성이 동시에 부각되는 이 시기에는, 성능과 안전을 통합한 평가 벤치마크의 부재가 가장 큰 갭으로 남는다.