...
**모순 관계**는 명시적이지 않으나 암묵적 긴장이 존재한다. 다수 에이전트의 비평·투표가 의사결정 품질을 높인다는 가정(Self-Driving Portfolio, MARCH)과, 동일한 집단 과정이 사회적 편향을 재현한다는 발견(Social Dynamics) 사이의 긴장이 대표적이다. 다중 에이전트 검증이 편향을 상쇄한다는 낙관과, 집단 과정 자체가 새로운 편향을 생성한다는 비판이 정면으로 대치된다.

## 연구 트렌드와 미래 방향

세 가지 트렌드가 식별된다. **(1) 에이전트 from 도구 to 인프라**: Agentic FL이 에이전트를 ML 파이프라인의 제어 평면으로 배치한 것처럼, 에이전트가 응용 도구가 아닌 시스템 인프라 자체로 자리잡는 흐름이 가속한다. **(2) 사회적 안전성의 중심화**: 기만, 동조, 귀인 편향 등 사회심리학적 취약성이 MAS 안전 연구의 핵심 의제로 부상하며, 기술적 강건성과 사회적 강건성의 통합 평가 체계가 필요하다. **(3) 인간 역할의 재정의**: Self-Driving Portfolio의 '감독자' 모델, Biased Error Attribution의 귀인 문제 등은 인간이 MAS와 어떤 관계를 맺어야 하는지에 대한 근본적 질문을 제기한다.

미래 연구는 다중 에이전트 시스템의 **사회적 면역 체계(social immune system)** 설계—편향이 자가 증폭되지 않도록 구조적으로 방어하는 메커니즘—와, 자기 개선 루프의 안전한 경계 설정 문제에 집중할 것으로 전망된다.