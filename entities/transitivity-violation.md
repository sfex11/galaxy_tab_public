# transitivity-violation

**카테고리**: 미분류
**생성일**: 2026-05-10

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-05-10-why-global-llm-leaderboards-are-misleading-small-p.md|Why Global LLM Leaderboards Are Misleading: Small Portfolios]]

### Clean Engineering, Unstable Measurement: A Preregistered Reliability F (2026-09-07)

추이성 위반에 시간축을 추가한다. 동일 시점 내 A>B, B>C, A<C라는 판단 집합 간 비일관성을 넘어, 같은 요청을 나중에 다시 보내면 순위 자체가 바뀌는 판독 불안정성이 관찰되어, judge 비일관성의 원인이 모델 판단 논리뿐 아니라 공유 추론 인프라의 상태 변화에도 있음을 시사한다.
