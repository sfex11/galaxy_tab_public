# preregistered-measurement-audit

**카테고리**: 미분류
**생성일**: 2026-09-06

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-09-06-clean-engineering-unstable-measurement-a-preregist.md|Clean Engineering, Unstable Measurement: A Preregistered Rel]]

### Clean Engineering, Unstable Measurement: A Preregistered Reliability F (2026-09-07)

사전등록 방법론의 실제 작동 사례를 제공한다. 두 캠페인에서 모든 임계값을 사전 고정하고 측정 전 도구 검증을 의무화했는데, 두 캠페인 모두 도구 검증 단계를 통과하지 못해 사전등록이 '연구 실패의 조기 차단 장치'로 기능함을 입증한다. 사전등록이 서술적 규범이 아니라 실행 가능한 품질 게이트임을 보여준다.
