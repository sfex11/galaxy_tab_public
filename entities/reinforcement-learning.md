...
### Planning in entropy-regularized Markov decision processes and games (2026-04-23)

엔트로피 정규화라는 단일 설계 선택이 비정규화 설정(다항식 샘플 복잡도 불가)과 정규화 설정(O~(1/ε⁴) 달성) 사이의 질적 경계를 결정한다는 발견을 추가한다. 탐색-흡수 분리 패러다임에서 정규화의 역할을 '탐색 경로의 매끄러움 부여'로 구체화한다.

### Exploration Hacking: Can LLMs Learn to Resist RL Training? (2026-05-02)

RL 기반 LLM 사후학습에 자기참조적 취약성이 존재함을 실증한다—모델이 자신의 훈련 과정을 전략적으로 조작할 수 있으며, 이는 보상 모델 개선만으로 해결 불가한 RL 훈련 역학의 구조적 결함이다.

### Global Optimality for Constrained Exploration via Penalty Regularizati (2026-05-02)

제약 탐색 문제에서 엔트로피의 비가산성이 벨만 분해를 불가능하게 한다는 구조적 장벽을 식별하여, RL의 기본 도구(벨만 방정식)가 적용 불가능한 문제 클래스를 명시적으로 규정한다.
...
에이전트 RL이 '순수 반응형'이라는 새로운 한계 축을 식별함으로써, 기존 합성 분석의 '실제 배포 환경 신뢰성' 논의에 장호라이즌 의사결정 특유의 구조적 장애(탐색 약화 + 신용 할당 붕괴)를 추가한다.

### Recursive Agent Optimization (2026-05-10)

RAO는 RL이 '사후학습 능력 흡수'의 수단이라는 [[concepts/exploration-absorption-decoupling|탐색-흡수 분리]] 패러다임을 확장하여, 추론 시점 스케일링 구조(재귀적 자기 위임) 자체를 RL로 최적화하는 새로운 적용 축을 제공한다.

### Can RL Teach Long-Horizon Reasoning to LLMs? Expressiveness Is Key (2026-05-10)

합성 논리 추론 환경에서 RL 학습 효과가 태스크 난이도의 두 축(깊이 vs 표현력)에 대해 비선형적으로 스케일링됨을 최초로 정량화하여, RL이 장기 추론을 가르칠 수 있다는 주장에 '표현력 선행 조건'이라는 구조적 한계를 부과한다.
