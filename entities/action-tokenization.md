# action-tokenization

**카테고리**: 미분류
**생성일**: 2026-09-06

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-09-06-continuous-actions-from-discrete-minds-latent-alig.md|Continuous Actions from Discrete Minds: Latent-Aligned Plann]]

### Continuous Actions from Discrete Minds: Latent-Aligned Planning for En (2026-09-07)

잔여 벡터 양자화 VQ-VAE를 액션 토크나이저의 구체적 구현으로 제시하여, 이 개념이 연속 행동 공간을 이산 토큰 어휘로 번역하는 '표현 계약'임을 명확히 한다. 토크나이저 설계가 곧 물리 제약의 인코딩 방식을 결정함을 보여준다.

### Show-Harness: Just a VLM Agent Can Play Robots (2026-09-11)

행동 토큰화의 대상을 저수준 모터 명령이 아닌 의도 수준의 이산적 의미 행동 단위로 격상시킨다. VLM이 자연스럽게 추론할 수 있는 추상화 수준에서 행동 어휘를 정의하는 것이 핵심 설계 변수임을 보여준다.
