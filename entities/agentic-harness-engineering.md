# agentic-harness-engineering

**카테고리**: 미분류
**생성일**: 2026-05-02

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-05-02-crab-a-semantics-aware-checkpointrestore-runtime-f.md|Crab: A Semantics-Aware Checkpoint/Restore Runtime for Agent]]

### Crab: A Semantics-Aware Checkpoint/Restore Runtime for Agent Sandboxes (2026-05-03)

C/R 런타임이 하네스의 핵심 구성요소(결함 내성·롤백·분기)를 직접 구현하는 인프라 계층으로, 하네스 엔지니어링의 범위를 애플리케이션 로직에서 실행 환경까지 확장함을 시사한다.

### RunAgent: Interpreting Natural-Language Plans with Constraint-Guided E (2026-05-05)

제약과 rubric이 플랜 언어 수준에서 내재화됨으로써, 하네스의 실행 제어 기능이 애플리케이션 로직에서 플랜 표현 자체로 이동하는 설계 패턴을 제시한다.

→ [[sources/2026-05-05-runagent-interpreting-natural-language-plans-with-.md|상세 보기]]

### Position: agentic AI orchestration should be Bayes-consistent (2026-05-05)

하네스 설계가 준수해야 할 의사결정 이론적 기준을 제공하여, 기존 '관측 가능성 주도 진화'의 평가 축을 효율성에서 일관성+효율성으로 확장한다.

→ [[sources/2026-05-05-position-agentic-ai-orchestration-should-be-bayes-.md|상세 보기]]

### Design Conductor 2.0: An agent builds a TurboQuant inference accelerat (2026-05-08)

Design Conductor 2.0은 하네스 엔지니어링이 단발성 최적화가 아닌 모델 발전 주기와 동기화된 지속적 진화 과정임을 결정적 실증으로 제공한다. 4개월간 프론티어 모델 진화와 하네스 개선이 병행되어 80배 규모 확장을 달성한 경로를 구체화하여, 기존 '관측 가능성 주도 진화' 개념을 '공진화적 하네스 설계'로 확장한다.

→ [[sources/2026-05-08-design-conductor-20-an-agent-builds-a-turboquant-i.md|상세 보기]]
