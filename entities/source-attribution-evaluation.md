# source-attribution-evaluation

**카테고리**: 미분류
**생성일**: 2026-05-10

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-05-10-cited-but-not-verified-parsing-and-evaluating-sour.md|Cited but Not Verified: Parsing and Evaluating Source Attrib]]

### Cited but Not Verified: Parsing and Evaluating Source Attribution in L (2026-05-11)

출처 귀속 평가를 접근성·관련성·사실 일관성의 3차원으로 분해하여, 인용이 첨부된 주장을 실제로 지지하는지를 평가하는 프레임워크로 정의를 구체화한다.

### ReCite: Agentic Reasoning for Faithful Citation (2026-09-10)

접근성·관련성·사실 일관성 3차원 프레임워크가 식별하는 '관련성≠지지성' 간극의 생성 측 대응책을 제공한다. 평가가 측정하는 실패 모드(접근 가능하지만 주장을 지지하지 않는 인용)를 에이전틱 생성 단계에서 직접 예방하는 상보 관계다.
