# certification-monitoring-discontinuity

**카테고리**: 미분류
**생성일**: 2026-09-06

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-09-06-clean-engineering-unstable-measurement-a-preregist.md|Clean Engineering, Unstable Measurement: A Preregistered Rel]]

### Clean Engineering, Unstable Measurement: A Preregistered Reliability F (2026-09-07)

인증-감시 단절의 제3의 축을 제공한다. 기존 '평가 기준 변화'와 '환경 변화' 단절에 더해, 측정 기기 자체가 시간에 따라 드리프트하는 '도구 드리프트' 단절을 추가하여, 인증 무효화의 원인이 측정 대상과 측정 도구 양쪽에서 발생할 수 있음을 명시한다.

→ [[sources/2026-09-07-clean-engineering-unstable-measurement-a-preregist.md|상세 보기]]

### SpecGuard: Inference-Time Backdoor Detection For Free (2026-09-13)

단절의 실용적 완화 경로를 제공한다. 런타임 감시 비용이 0에 수렴하면 잦은 업데이트 모델에 대한 지속 감시가 경제적으로 성립하여, 인증 유효기간 문제의 완화 조건이 명시된다.

→ [[sources/2026-09-13-specguard-inference-time-backdoor-detection-for-fr.md|상세 보기]]
