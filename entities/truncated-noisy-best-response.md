# truncated-noisy-best-response

**카테고리**: 미분류
**생성일**: 2026-09-12

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-09-12-truncated-noisy-best-response-algorithms-toward-ga.md|Truncated Noisy Best-Response Algorithms: Toward Game Theore]]

### Truncated Noisy Best-Response Algorithms: Toward Game Theoretic Learni (2026-09-13)

본 논문이 이 엔티티의 원천 소스로서 정의를 확립한다: 최악 보장 평형의 동학적 불안정성을 절단(truncation)과 잡음(noise)으로 활용하여, 성능 개선과 하한 유지를 동시에 달성하는 알고리즘 패밀리. 에이전트별 유연 특성화가 설계의 핵심 자유도임을 명시한다.
