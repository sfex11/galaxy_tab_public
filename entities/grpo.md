# grpo

**카테고리**: 미분류
**생성일**: 2026-05-10

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-05-10-beyond-negative-rollouts-positive-only-policy-opti.md|Beyond Negative Rollouts: Positive-Only Policy Optimization ]]

### Beyond Negative Rollouts: Positive-Only Policy Optimization with Impli (2026-05-11)

음성 롤아웃 기반 단순 advantage 추정이 실제로는 검증 불가능한 출력의 노이즈를 흡수 과정에 주입하여 탐색-흡수 분리의 전제를 훼손시킨다는 비판적 관점을 제공한다. OpenVLThinkerV2 등 GRPO 기반 모델의 학습 효율 재평가를 위한 구체적 근거가 된다.

### Bellman Policy Optimization (2026-09-16)

GRPO가 그룹 기준선으로 크리틱을 우회한 것이 경험적 선택이었음을 보완하여, 동일한 크리틱 프리 목표를 PMD 기반으로 도출하는 이론적 대안 경로를 제시한다.

→ [[sources/2026-09-16-bellman-policy-optimization.md|상세 보기]]

### A Zeroth-Order Paradigm for LLM Preference Alignment (2026-09-18)

그룹 내 상대 advantage 구성과 병렬되는 쌍 단위 비교 신호의 사례를 제공한다. 두 방법 모두 절대 보상 없이 상대 비교만으로 업데이트 방향을 결정하며, ComPO는 이를 우도 공간이 아닌 비교 공간에서 수행함으로써 제로스-오더 상대 신호 방법론의 원자적 하한을 보여준다.

→ [[sources/2026-09-18-a-zeroth-order-paradigm-for-llm-preference-alignme.md|상세 보기]]
