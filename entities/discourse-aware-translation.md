# discourse-aware-translation

**카테고리**: 미분류
**생성일**: 2026-09-04

## 정의

_Wiki 축적 중_

## 관련 논문

- [[sources/2026-09-04-discosign-discourse-aware-text-to-sign-language-gl.md|DiscoSign: Discourse-Aware Text to Sign Language Gloss Trans]]

### Translation as a Decision Space: A Multi-Agent Perspective on Low-Reso (2026-09-06)

DiscoSign의 담화 인지 축과 본 논문의 대안 탐색 축이 병렬을 이루며, 번역이 정적 매핑에서 맥락 인지와 대안 탐색을 모두 요구하는 동적 과제로 확장되는 공통 흐름을 강화한다.

### Translation as a Decision Space: A Multi-Agent Perspective on Low-Reso (2026-09-07)

DiscoSign의 담화 인지 축과 본 논문의 대안 탐색 축이 병렬을 이루며, 번역이 정적 매핑에서 맥락 인지와 대안 탐색을 모두 요구하는 동적 과제로 확장되는 공통 흐름을 강화한다.

→ [[sources/2026-09-07-translation-as-a-decision-space-a-multi-agent-pers.md|상세 보기]]
