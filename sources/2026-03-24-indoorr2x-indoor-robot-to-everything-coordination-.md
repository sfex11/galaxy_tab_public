...
### LLM 기반 계획
LLM을 활용한 계획(planning) 모듈이 핵심 구성요소로, 이기종 센서 데이터를 통합하고 로봇의 행동을 조율한다. 이는 [[llm-agent]] 연구에서 LLM이 도구 사용과 환경 상호작용을 조율하는 패러다임의 자연스러운 확장이며, [[embodied-ai]]와 [[spatial-reasoning]] 분야를 IoT 인프라 수준으로 확장한 사례이다.
...
- [[multi-agent-system]] 관점에서 로봇-센서 간 이기종 협업이라는 새로운 차원을 추가한다.
