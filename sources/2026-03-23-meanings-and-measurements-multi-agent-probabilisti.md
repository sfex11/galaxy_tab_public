...
기존 Wiki의 [[VLM]] 및 [[vision-language-model]] 연구와 직접적으로 연결되며, VLM의 공간 추론 한계를 보완하는 방향을 제시한다. [[spatial-reasoning]] 개념의 확장으로, 단순 공간 이해를 넘어 메트릭 수준의 정밀한 공간 추론을 요구한다. [[embodied-ai]] 분야에서 언어-행동 연결의 구체적 구현이며, [[multi-agent-system]] 패러다임을 로봇 내비게이션에 적용한 사례다. AgentVLN과 같은 기존 에이전트 기반 내비게이션 연구와도 밀접한 관련이 있으나, 확률적 그라운딩이라는 차별화된 접근을 취한다.
