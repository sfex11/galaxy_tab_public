...
### Wiki 연결점
본 연구는 [[reinforcement-learning]]과 [[reasoning-chain]] 개념의 교차점에 위치한다. 특히 Nemotron-Cascade 2의 cascade RL 접근([[cascade-reinforcement-learning]])과 비교할 때, BCR은 단일 단계로 단순화한 점이 차별적이다. 또한 [[scaling-laws]] 관점에서 토큰 소비에 대한 새로운 스케일링 법칙을 제시하며, [[curriculum-learning]]의 다단계 접근을 불필요하게 만드는 대안을 제공한다. [[training-data-pruning]]과도 연결되는데, 두 연구 모두 '더 적게 학습하여 더 나은 결과'라는 철학을 공유한다.
