...
### Wiki 연결점
이 연구는 [[concepts/llm-benchmark.md|llm benchmark]] 계열의 새로운 축을 추가한다. 기존 FinTradeBench가 금융 추론을, SOL-ExecBench가 GPU 커널 성능을 측정했다면, StackRepoQA는 **코드 이해의 공간적 범위(scope)**를 벤치마크 차원으로 확장한다. 또한 [[concepts/long-context.md|long context]] 연구와 직접적으로 관련되는데, 레포지토리 전체를 컨텍스트 윈도우에 넣어야 하는 설정은 긴 컨텍스트 처리 능력을 필수적으로 요구한다. [[concepts/retrieval-augmented-generation.md|retrieval augmented generation]] 파이프라인의 코드 도메인 적용 가능성도 시사하며, 최근의 [[concepts/code-generation.md|code generation]] 벤치마크들이 생성에 집중한 것과 달리 **이해(comprehension)** 측면을 강조한다는 점에서 차별화된다.
