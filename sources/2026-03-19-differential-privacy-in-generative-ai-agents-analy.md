...
본 연구는 [[concepts/ai-safety.md|ai safety]] 개념과 직접적으로 연결된다. TraceSafe가 LLM 가드레일의 도구 사용 시 안전성을 평가했다면, 이 논문은 **데이터 접근 단계에서의 프라이버시 보호 메커니즘**을 다룬다. 또한 [[entities/llm-agent.md|llm agent]] 엔티티와 관련하여, 에이전트가 외부 데이터에 접근하는 과정에서 발생하는 새로운 위험 차원을 조명한다. [[concepts/tool-use.md|tool use]] 개념과도 연결되는데, 에이전트의 도구 사용(데이터베이스 쿼리)이 프라이버시 공격 표면을 확장시키는 문제를 다루기 때문이다.
