...
이 시스템의 핵심 발견은 하네스와 기저 모델의 **공진화(co-evolution)**가 복합적 성능 향상을 낳는다는 것이다. 2025년 12월~2026년 4월 사이 프론티어 모델의 진화만으로도 성능이 향상되었지만, 하네스 자체를 동시에 개선하면 두 축의 개선이 곱셈적으로 결합한다. 이는 [[entities/agentic-harness-engineering.md|하네스 엔지니어링]]이 정적 최적화가 아닌 모델 발전과 동기화된 진화적 과정이어야 함을 실증한다.

## 작업 규모의 비선형 확장

CPU 설계(12시간)에서 추론 가속기 설계(80시간)로의 80배 규모 확장이 시간 측면에서는 ~6.7배 증가에 그친다. 이는 하네스가 작업 규모에 대해 아임(linear) 이하의 시간 증가를 달성함을 시사하며, [[concepts/harness-as-hidden-variable.md|하네스를 은닉 변수로 처리]]하는 기존 벤치마크가 하네스 설계 품질이 작업 규모 확장성을 어떻게 결정하는지를 보여준다.

## 관련 문제

- 하드웨어 설계는 [[concepts/sandbox-liveworld-gap.md|샌드박스-실세계 격차]]의 극단적 사례—설계의 검증이 실제 패브리케이션에서만 완료됨
- [[concepts/autoresearch.md|오토리서치]]의 스코프를 소프트웨어에서 하드웨어 아티팩트로 확장하며, [[concepts/artifact-bound-optimization.md|아티팩트 구속 최적화]]가 반드시 나쁜 것은 아님을 보여줌
...
- → [[sources/2026-05-08-executable-world-models-for-arc-agi-3-in-the-era-o]]: 두 논문 모두 코딩 에이전트가 복잡한 문제를 자율적으로 해결하는 시스템을 다루며, 하나는 하드웨어 설계를, 다른 하나는 ARC-AGI 벤치마크를 대상으로 한다.
- → [[sources/2026-05-07-physics-grounded-multi-agent-architecture-for-trac]]: 두 논문 모두 물리적·공학적 도메인에서의 다중 에이전트 자율 시스템을 다루며, 하나는 제조 공정의 위험 인지 의사결정을, 다른 하나는 하드웨어 설계 자동화를 목표로 한다.
