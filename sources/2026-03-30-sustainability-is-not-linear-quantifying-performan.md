...
### 기존 Wiki와의 연결
본 연구는 [[concepts/differential-privacy.md|differential privacy]] 개념과 직접 연결되며, 온디바이스 처리를 통한 프라이버시 보호라는 대안적 접근을 탐구한다. [[concepts/model-compression.md|model compression]]과 [[concepts/model-pruning.md|model pruning]] 개념은 엣지 배포를 위한 모델 경량화 기법으로서 본 논문의 양자화 실험과 밀접하게 관련된다. 또한 [[concepts/hardware-aware-benchmarking.md|hardware aware benchmarking]]의 관점에서 실제 하드웨어 제약을 고려한 벤치마킹 방법론을 공유하며, [[concepts/llm-benchmark.md|llm benchmark]] 범주에서 추론 효율성 측면의 새로운 평가 차원을 추가한다. [[concepts/speculative-decoding.md|speculative decoding]]과 같은 추론 가속 기법들이 엣지 환경에서 어떤 에너지-성능 프로필을 보이는지에 대한 후속 연구로 이어질 수 있다.
...
---
**관련**: [[entities/differential-privacy.md|differential privacy]]
