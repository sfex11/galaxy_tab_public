...
이 연구는 [[concepts/post-training.md|post training]] 개념과 직접적으로 관련되며, 기존 모델의 능력을 확장하기 위해 추가 학습을 적용하는 접근법을 공유한다. 또한 [[concepts/audio-language-model.md|audio language model]] 및 [[concepts/audio-question-answering.md|audio question answering]] 분야의 확장으로, 음성 모델의 제어 가능성이라는 새로운 축을 제시한다. LLM Agent 엔티티와도 연결되는데, 음성 에이전트가 실제 환경에서 시간 제약 하에 동작해야 하는 요구사항을 다루기 때문이다.
