...
### 자연어-형식언어 번역의 제3경로
기존 [[concepts/natural-language-to-formal-language.md|natural language to formal language]] 논의가 'NL→LTL 등 형식 명세'의 완전 번역과 '의미론적 번역의 어려움'을 다루었다면, RunAgent는 완전한 형식화가 아닌 **에이전트 언어(agentic language)**라는 중간 표현을 도입한다. 이는 NL의 표현력을 유지하면서 프로그래밍의 결정론성을 부여하는 제3경로로, 기존의 2분법(NL vs 형식언어)을 해체한다.

### 프로세스 제어의 플랜 수준 확장
[[concepts/process-control-architecture.md|process control architecture]]가 Box Maze에서 추론 체인 내부의 무결성을 아키텍처 수준에서 강제했다면, RunAgent는 이를 **플랜 실행 계층**으로 확장한다. 단계별 rubric 검증이 스텝 수준 검증 디코딩([[concepts/step-level-verification-decoding.md|step level verification decoding]])의 실제 구현 사례가 되며, 제어 구조(IF/GOTO)가 프로세스 제어를 플랜 의미론에 직접 매핑한다.

### 알고리즘-시스템 번역 간극의 중간층 해법
[[concepts/algorithm-system-translation-gap.md|algorithm system translation gap]]이 '의미론적 의존 그래프를 OS 메커니즘으로 번역할 때의 비효율'을 문제 삼았다면, RunAgent의 에이전트 언어는 이 번역 간극을 **실행 의미론의 명시적 중간 표현**으로 완화하는 시도다. 제어 구조가 플랜의 의미론을 코드가 아닌 제약으로 인코딩한다.

## 연결점
- [[entities/scientific-workflow-agent.md|scientific workflow agent]]: 연구 질문→워크플로우 자동화가 '의미론적 번역'에 집중했다면, RunAgent는 번역된 플랜의 **실행 신뢰성**에 집중하여 보완적 축을 형성한다.
- [[concepts/sandbox-liveworld-gap.md|sandbox liveworld gap]]: 플랜 실행의 결정론성 강화가 샌드박스-실세계 간극을 구조적으로 축소하는 경로를 제공한다.
- [[entities/agentic-harness-engineering.md|agentic harness engineering]]: 제약과 rubric이 하네스의 핵심 구성요소(실행 제어)를 플랜 언어 수준에서 내재화한다.
