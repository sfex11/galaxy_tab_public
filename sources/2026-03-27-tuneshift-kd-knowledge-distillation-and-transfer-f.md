...
### 기존 Wiki와의 연결
- **[[concepts/knowledge-distillation.md|knowledge distillation]]**: TuneShift-KD는 전통적 knowledge distillation을 확장하여, 단순히 큰 모델→작은 모델 압축이 아닌 **아키텍처 간 전문 지식 이전**이라는 새로운 축을 제시한다. F2LLM-v2 등 기존 distillation 연구가 모델 압축에 초점을 맞춘 반면, 본 연구는 지식의 이식성(portability)에 집중한다.
- **[[concepts/post-training.md|post training]]**: LoRA 기반 PEFT 후 지식 전이라는 점에서 Nemotron-Cascade 2의 post-training 파이프라인과 보완적 관계에 있다.
- **[[concepts/transfer-learning.md|transfer learning]]**: 모델 간 전이학습의 새로운 패러다임으로, 기존 transfer-learning 개념을 LLM 시대에 재정의한다.
- **[[concepts/model-compression.md|model compression]]**: 모델 압축과 달리 아키텍처가 다른 모델 간 지식 이동이라는 차별점이 있으나, 기술적 기반을 공유한다.
