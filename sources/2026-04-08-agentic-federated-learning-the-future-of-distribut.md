...
기존 FL은 클라이언트 선택, 집계 전략, 자원 배분 등을 사전 정의된 규칙으로 처리하지만, Agentic-FL에서는 LLM 에이전트가 실시간으로 시스템 상태를 관찰하고 적응적 의사결정을 수행한다. 이는 [[concepts/federated-learning.md|federated learning]]의 근본적 한계인 non-IID 데이터 분포와 시스템 이질성 문제를 동적으로 해결하려는 시도이다.

### 기존 연구와의 연결

- **LLM Agent + 최적화**: [[entities/llm-agent.md|llm agent]] 엔티티의 활용 범위가 NLP/코드 생성을 넘어 분산 시스템 오케스트레이션으로 확장되는 사례. [[concepts/multi-agent-system.md|multi agent system]]의 조율(coordination) 문제와도 직결된다.
- **연합학습 보안**: 기존 [[concepts/federated-learning.md|federated learning]] 관련 논문(FL-PBM 등)이 백도어 공격 방어에 집중했다면, 본 논문은 효율성과 적응성 관점에서 접근한다.
- **에이전트 기반 의사결정**: [[concepts/ai-decision-making.md|ai decision making]], [[concepts/agent-coordination.md|agent coordination]] 개념과 밀접하며, 에이전트가 자원 할당과 클라이언트 선택을 자율적으로 수행한다는 점에서 [[concepts/task-allocation.md|task allocation]]과도 관련된다.
- **비정상 역학 대응**: 시스템 역학의 예측 불가능성에 대응한다는 점에서 [[concepts/non-stationary-dynamics.md|non stationary dynamics]] 개념과 연결된다.
