...
이 연구는 기존 Wiki의 [[concepts/world-model.md|world model]] 개념과 직접적으로 연결된다. VectorWorld가 자율주행을 위한 스트리밍 월드 모델을 다뤘다면, ActionParty는 게임 환경에서의 인터랙티브 월드 모델로 확장한다. [[concepts/diffusion-model.md|diffusion model]] 및 [[concepts/video-diffusion.md|video diffusion]] 개념의 응용 범위를 단일 주체에서 다중 주체로 넓히는 중요한 진전이다.

[[concepts/multi-agent-system.md|multi agent system]] 관점에서도 주목할 만하다. 기존 다중 에이전트 연구가 주로 텍스트/계획 수준의 협조를 다뤘다면, ActionParty는 **시각적 생성 레벨**에서의 다중 에이전트 제어를 시도한다. 또한 [[concepts/3d-human-reconstruction.md|3d human reconstruction]]의 AHOY나 [[concepts/humanoid-agent.md|humanoid agent]]의 Visually-grounded Humanoid Agents와 같이 시각적 에이전트 표현을 다루는 연구들과 상호보완적이다. ActionParty가 생성 측면을, 후자들이 인식/제어 측면을 각각 담당하는 구도이다.
