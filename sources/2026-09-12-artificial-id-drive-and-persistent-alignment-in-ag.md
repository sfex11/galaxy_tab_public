...
본 논문은 에이전트 제어의 근본 위상을 역전시킨다. 목표·재시도·검증·정지 규칙 등 행동 전이를 외부 하네스가 '손으로' 명세하는 현행 패러다임([[entities/agentic-harness-engineering.md|agentic harness engineering]])에 대해, 계속/정지/변경을 판단하는 적응적 내부 구동체 artificial id를 제안한다. Freud적 은유로 읽으면 통제가 하네스 계층(외부 초자아)에서 에이전트 내부 본능 계층으로 이동하는 것이다.

기존 Wiki와의 관계: (1) [[concepts/harness-native-training.md|harness native training]]과 반대 방향인 '내부-네이티브 제어' 축을 연다. (2) [[concepts/termination-guarantee-problem.md|termination guarantee problem]]의 정지 규칙을 외부 명세에서 내부 구동으로 재배치한다. (3) [[concepts/interpretive-vs-structural-enforcement.md|interpretive vs structural enforcement]]의 해석적 강제를 넘어 규칙의 생성·적용 자체가 내부화된 제3 범주를 연다. (4) task 경계를 넘어 consequential state를 유지한다는 전제는 [[concepts/episodic-persistent-state-gap.md|episodic persistent state gap]]의 해소 방향을 제시한다. (5) 계속/변경 판단은 [[concepts/goal-fixation-meta-decision.md|goal fixation meta decision]]을 내부 구동으로 조작화한 것이다.

위험 축: 내부 구동의 자기 판단은 [[concepts/pseudo-alignment-by-self-consistency.md|pseudo alignment by self consistency]]와 [[concepts/designer-foresight-boundary.md|designer foresight boundary]]의 침식을 구조적으로 수반하며, 'Persistent Alignment'라는 표제는 정렬이 세션 경계를 넘어 유지되어야 한다는 [[concepts/alignment-scope-erosion.md|alignment scope erosion]]의 새 국면을 시사한다. Petri-dish 최소 실험은 [[concepts/metacognition.md|metacognition]]적 판단의 형성 과정을 외부 관측 가능하게 하는 방법론적 장치이며, [[concepts/autonomy-spectrum.md|autonomy spectrum]]의 반성적 단계 구현 후보로 위치한다.
