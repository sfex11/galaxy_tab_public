...
### 기존 연구와의 관계
본 연구는 text-to-image 생성 패러다임을 단일 이미지 출력에서 편집 가능한 다층 미디어 문서로 확장한다. 기존 Wiki의 [[concepts/text-to-image.md|text to image]] 개념과 직접적으로 연관되며, diffusion 모델 기반 생성이라는 점에서 생성 모델 연구의 응용 범위를 그래픽 디자인 영역으로 넓힌 사례이다. 또한 자연어 프롬프트로 복잡한 구조적 출력을 제어한다는 점에서 [[concepts/prompt-engineering.md|prompt engineering]] 개념과도 연결된다.
