...
### Wiki 연결점
이 연구는 [[entities/llm-agent.md|llm agent]] 생태계의 **환경 인프라** 문제를 다룬다는 점에서 독특한 위치를 차지한다. [[clawbench-can-ai-agents-complete-everyday-online-t|ClawBench]]가 웹 에이전트의 일상 태스크 수행 능력을 평가하는 벤치마크라면, Gym-Anything은 그러한 벤치마크 자체를 자동으로 생성하는 메타 프레임워크에 해당한다. [[concepts/web-agent-evaluation.md|web agent evaluation]] 개념과 직접 연결되며, [[concepts/tool-use.md|tool use]] 연구의 범위를 웹 브라우저 너머 임의의 데스크탑 소프트웨어로 확장한다.
