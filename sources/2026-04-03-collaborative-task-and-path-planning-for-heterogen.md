...
기존 Wiki의 [[concepts/multi-robot-coordination.md|multi robot coordination]] 개념과 직접 연결되며, ADMM 기반 분산 MPC 연구가 안전 제약 하의 다중 로봇 협업을 다뤘다면, 본 연구는 MARL(Multi-Agent RL) 기반으로 확장성 문제를 해결한다. [[concepts/multi-agent-path-planning.md|multi agent path planning]]의 경로 계획 문제를 작업 할당(task allocation)과 통합한 점이 차별점이다. [[concepts/reinforcement-learning.md|reinforcement learning]] 엔티티의 다중 에이전트 확장 사례로, PPO를 협업 환경에 적용한 구체적 인스턴스를 제공한다. 고전적 계획 알고리즘의 확장성 한계를 MARL로 극복하려는 시도는 [[concepts/graph-based-planning.md|graph based planning]] 및 [[concepts/computational-complexity.md|computational complexity]] 논의와도 맞닿아 있다. [[concepts/multi-objective-optimization.md|multi objective optimization]]의 관점에서 활용도와 과학적 가치라는 복수 목표 간 균형 문제도 내포한다.
