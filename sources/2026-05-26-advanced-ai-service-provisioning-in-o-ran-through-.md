...
## 관련 엔티티
- [[algorithm-system-translation-gap]]: 이 논문이 O-RAN이라는 구체적 인프라에서 이 간극의 구체적 해결 사례임
- [[agent-os-semantic-gap]]: LLM이 RAN 프로토콜을 이해하고 코드로 재생성하여 OS 의미론 간극을 해소함
- [[code-as-agent-harness]]: LLM이 xApp 개발 설계를 직접 수행하는 주체로 전환하는 패러다임의 구체적 실현
- [[delta-checkpoint-rollback]]: 실시간 RAN 상태 관리에서 의미론적 최소화 델타 복원의 의미
- [[inference-time-behavior-control]]: 실시간 제어 컨트롤에 LLM 개입으로 의사결론적 제어 강제
- [[code-sandbox-confinement]]: 생성 코드의 실시간 안전 배포 보장
