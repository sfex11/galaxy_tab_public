...
기존 Wiki의 [[llm-agent]] 엔티티와 직접적으로 연결되며, [[multi-agent-system]] 개념의 사이버보안 도메인 적용 사례에 해당한다. [[ai-safety]] 개념과도 관련이 있는데, 사이버보안 리스크 관리 자체가 AI 시스템의 안전한 배포를 위한 기반이 되기 때문이다. 또한 에이전트 간 컨텍스트 공유 메커니즘은 [[tool-use]] 개념에서 다루는 에이전트의 외부 정보 활용 패턴과 맥을 같이한다.
