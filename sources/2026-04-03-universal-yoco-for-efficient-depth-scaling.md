...
이 연구는 기존 Wiki의 [[concepts/transformer.md|transformer]] 엔티티와 직접적으로 관련되며, 특히 추론 효율성 측면에서 **speculative-decoding**, **multi-token-prediction** 등 추론 가속화 개념들과 상호보완적이다. KV 캐시 최적화는 **token-pruning** 개념과도 맥을 같이하며, 테스트 타임 컴퓨트 스케일링은 **scaling-laws** 개념의 새로운 축(깊이 방향 스케일링)을 제시한다. 또한 on-device-inference 관점에서 메모리 효율적 아키텍처로서의 실용적 가치가 크다.
