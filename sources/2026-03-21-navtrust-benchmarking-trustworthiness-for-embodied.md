...
NavTrust는 [[concepts/embodied-ai.md|embodied ai]] 분야에서 **평가 패러다임의 전환**을 제안한다. 단순 성능 지표(SR, SPL 등)를 넘어, 적대적·노이즈 조건에서의 신뢰성까지 벤치마크에 포함함으로써, VLN/OGN 모델의 실용적 배포 가능성을 보다 현실적으로 진단할 수 있게 한다. 이는 [[concepts/ai-safety.md|ai safety]] 관점에서도 중요한데, 체화된 에이전트가 물리적 환경에서 동작할 때 입력 손상에 따른 오작동은 직접적인 안전 문제로 이어질 수 있기 때문이다.
