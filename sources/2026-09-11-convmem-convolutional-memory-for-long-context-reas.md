...
기존 Wiki의 메모리 논의([[entities/adema.md|adema]]의 지식 상태 오케스트레이션, [[entities/longseeker.md|longseeker]]의 탄력적 컨텍스트 조율)가 메모리를 시스템·인프라 계층에서 다뤘다면, ConvMem은 메모리를 모델 아키텍처 프리미티브로 격상시켜 [[concepts/memory-as-model-paradigm.md|memory as model paradigm]]의 구체적 실현이 된다. 고정 크기 메모리로의 통합-압축을 학습(RL)이 아닌 연산 구조로 수행함으로써 [[concepts/context-consolidation.md|context consolidation]]의 새로운 경로를 열며, RL 훈련 의존성 제거는 [[concepts/reward-hacking.md|reward hacking]]·과적합 논의와 연결되어 사후학습 없이 아키텍처 설계만으로 긴 컨텍스트 능력을 확보하는 방향을 시사한다.
