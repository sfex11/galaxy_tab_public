...
[[process-control-architecture]]의 Box Maze가 LLM 추론의 신뢰성을 프로세스 제어로 확보하려 했다면, 본 논문은 에이전트의 **행동 자체**에 마찰을 부여하는 더 포괄적 프레임워크를 제시한다. [[metacognition]] 관점에서 Act Wisely의 메타인지적 도구 사용과도 맞닿아 있으며, [[tool-use]]의 과도한 호출 문제를 구조적으로 억제하는 메커니즘을 제공한다. [[ai-safety]]의 TraceSafe가 가드레일 평가에 집중했다면, 본 연구는 가드레일이 필요해지기 전 단계에서 에이전트 행동을 제한하는 아키텍처 수준의 해법을 탐구한다.
