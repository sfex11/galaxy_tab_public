...
[[entities/terminal-universe.md|terminal universe]]가 궤적에서 터미널 환경을 유도하는 '재질의' 경로라면, 본 논문은 실제 애플리케이션을 래핑해 GUI+CLI를 공유 애플리케이션 상태로 결합하는 '실측 래핑' 경로를 제공하여 [[concepts/environment-absence-bottleneck.md|environment absence bottleneck]]을 하이브리드 축에서 해소한다. 핵심 구조적 통찰은 두 모달리티가 동일 상태에 대한 상이한 [[concepts/representation-contract.md|representation contract]]이라는 점이다 — GUI는 토큰 비용이 큰 시각 관찰을, CLI는 정밀·저비용 구조적 관찰과 행동을 제공하며, 능력 있는 에이전트는 연산 단위마다 인터페이스를 선택·조율해야 한다. 이는 [[concepts/hci-aai-optimization-divergence.md|hci aai optimization divergence]]의 구체적 실현이자, GUI 전용 평가가 실제 작업 단위와 어긋난다는 [[concepts/evaluation-deployment-unit-mismatch.md|evaluation deployment unit mismatch]]·[[concepts/benchmark-specification-gap.md|benchmark specification gap]]의 환경 측 실증이며, [[concepts/computer-use-agent.md|computer use agent]]의 능력 정의를 'GUI 조작'에서 '이질 인터페이스 조율'로 확장한다.
