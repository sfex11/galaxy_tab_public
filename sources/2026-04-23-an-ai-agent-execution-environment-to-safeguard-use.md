...
- → [[sources/2026-04-22-using-large-language-models-for-embodied-planning-]]: 둘 다 AI 에이전트의 안전 문제를 다루며, 하나는 실행 환경 수준의 아키텍처적 격리로 데이터 유출을 방지하고, 다른 하나는 체화 계획에서의 체계적 위험을 벤치마크로 노출한다.
- → [[sources/2026-04-24-interval-pomdp-shielding-for-imperfect-perception-]]: 둘 다 자율 에이전트에 대한 방어 메커니즘을 제안하며, 하나는 데이터 유출에 대해 실행 환경 격리로, 다른 하나는 행동 안전 위반에 대해 형식적 POMDP 실드링으로 대응한다.
