...
Wiki 관점에서 이 논문의 기여는 두 층위다. 첫째, 처리 단위의 구조적 부정합([[concepts/abstraction-layer-mismatch.md|abstraction layer mismatch]])을 번역 도메인에서 실증한다: 문장 단위 운영은 담화 수준 구조를 필연적으로 파괴하며, 이는 [[concepts/token-step-pipeline-hierarchy.md|token step pipeline hierarchy]]가 문서화한 입도 부정합 패턴의 언어학적 변형이다. 둘째, 공간 지시 해소는 개체의 공간 위치라는 상태가 담화 전체에서 일관되게 유지되어야 하는 지식 상태 관리 문제로, 첫 언급에서 설정된 공간 앵커가 이후 모든 참조의 의존 기반이 되는 [[concepts/semantic-dependency-graph.md|semantic dependency graph]]를 형성한다. 이는 ADEMA 계열의 [[concepts/knowledge-state-orchestration.md|knowledge state orchestration]]과 동형 구조를 언어 처리 도메인에서 보여준다. 주목할 점은 긴 컨텍스트 주입이 아닌 명시적 구조 모듈로 교차 문장 의존성을 처리한다는 점이다.

나아가 도메인 지식(언어학)이 시스템 설계 구조를 결정하는 [[concepts/natural-language-to-formal-language.md|natural language to formal language]] 방향의 성공 사례이며, 형식 문법 기반 번역 평가 연구(Evaluating In-Context Translation)와 담화 수준에서 접점을 가진다.
