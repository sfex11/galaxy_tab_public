...
### 기존 연구와의 연결
- [[concepts/llm-benchmark.md|llm benchmark]] 계열 연구들이 주로 정확도·추론 품질을 측정하는 반면, BAS는 **신뢰도의 의사결정 유용성**이라는 새로운 축을 제시한다.
- [[concepts/metacognition.md|metacognition]] 연구(특히 "Causal Evidence that Language Models use Confidence to Drive Behavior")와 직접적으로 연결된다. 메타인지가 LLM 내부 신뢰도 활용 메커니즘을 탐구한다면, BAS는 그 신뢰도의 **외부 의사결정 가치**를 정량화한다.
- [[concepts/ai-safety.md|ai safety]] 관점에서 기권 능력은 안전한 배포의 핵심 요소이며, TraceSafe 등 가드레일 연구와 보완적이다.
- [[concepts/reasoning-integrity.md|reasoning integrity]] 연구(Box Maze 등)가 추론 과정의 신뢰성을 다룬다면, BAS는 추론 결과에 대한 신뢰도 표현의 신뢰성을 다룬다.
- 금융 도메인의 [[concepts/financial-reasoning.md|financial reasoning]] 벤치마크(FinTradeBench)처럼 위험 관리가 중요한 분야에서 BAS의 적용 가치가 높다.
