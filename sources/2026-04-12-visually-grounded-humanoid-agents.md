...
이 논문은 [[embodied-ai]] 분야의 핵심 연구로, 시각 정보만으로 행동하는 에이전트라는 점에서 [[visual-grounding]]과 [[vision-language-model]] 연구와 밀접하다. 특히 시각 기반 로코모션 정책은 [[visual-locomotion]]과 [[humanoid-agent]] 개념의 직접적 확장이며, 강화학습 기반 제어라는 점에서 [[reinforcement-learning]]과도 연결된다.

[[3d-scene-understanding]]과 [[3d-human-reconstruction]] 연구가 환경 인식과 인체 모델링의 기반을 제공하며, 본 논문은 이를 능동적 행동 생성으로 확장한다. 또한 스크립트 없이 새로운 환경에 자율적으로 적응한다는 점에서 [[distribution-shift]] 대응 및 [[continual-learning]] 관점에서도 의미가 있다.
...
이전에 등록된 동일 제목 논문([[sources/2026-04-11-visually-grounded-humanoid-agents.md]])의 업데이트 버전으로, [[diffusion-model]] 기반 모션 생성과 RL 기반 비전 정책의 결합이 핵심 아키텍처로 추정된다.
