...
이 연구는 기존 Wiki의 [[prompt-engineering]] 개념과 직접적으로 연관되며, LLM의 코드 생성 능력 평가라는 점에서 [[llm-benchmark]]와도 접점을 가진다. 또한 도메인 특화 지식 주입 방식에 대한 논의는 [[knowledge-distillation]], [[post-training]] 개념과 상보적 관계에 있다. 빠르게 변화하는 API에 대한 적응이라는 측면에서는 [[tool-use]] 개념의 확장으로도 볼 수 있으며, 코드 생성 품질 벤치마크 설계는 EvaluatingLLM-BasedTestGenerat 논문과 유사한 평가 프레임워크를 공유한다.
