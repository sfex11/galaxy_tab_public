...
LLM이 테스트를 생성할 때 진정한 프로그램 의미론(semantics) 이해에 기반하는지 여부는 실용적으로 중요한 문제이다. 만약 패턴 매칭에 의존한다면, 코드가 변경될 때 커버리지 감소, 회귀 미탐지, 결함 미발견 등의 약점이 드러날 수 있다. 이는 [[concepts/llm-benchmark.md|llm benchmark]] 연구들이 정적 스냅샷에서의 성능만 측정하는 한계를 보완하는 시각이다.
...
- **[[entities/llm-agent.md|llm agent]]**: LLM 에이전트가 코드 생성 및 테스트 자동화에 활용되는 맥락에서, 생성된 테스트의 진화 적응성은 에이전트 신뢰성의 핵심 지표가 된다.
- **[[concepts/ai-safety.md|ai safety]]**: LLM 생성 코드의 품질 보증은 AI 안전성의 실질적 측면이며, 테스트가 실제 결함을 탐지하지 못하면 프로덕션 장애로 이어질 수 있다.
- **[[concepts/reasoning-chain.md|reasoning chain]]**: LLM이 테스트 생성 시 진정한 추론을 하는지 vs 패턴 매칭인지의 구분은, LLM 추론 능력 평가의 또 다른 렌즈를 제공한다.
