...
이 연구는 기존 Wiki의 [[concepts/video-vlm.md|video vlm]], [[concepts/vision-language-model.md|vision language model]], [[concepts/agentic-vlm.md|agentic vlm]] 개념과 직접적으로 연결된다. 특히 VideoSeek가 도구 기반 탐색(tool-guided seeking)으로 장시간 비디오를 처리한 것과 유사하게, LensWalk도 에이전트가 비디오 내 필요한 구간을 능동적으로 선택하는 전략을 취한다. VideoAtlas의 계층적 비디오 탐색과도 맥락을 공유하며, Act Wisely의 메타인지적 도구 사용 개념이 비디오 지각 계획에 적용된 사례로 볼 수 있다.

### 핵심 차별점

기존 video-vlm 연구들이 토큰 압축([[concepts/token-pruning.md|token pruning]])이나 계층적 표현([[concepts/hierarchical-representation.md|hierarchical representation]])으로 효율성을 추구한 반면, LensWalk는 에이전트의 추론 과정 자체가 지각 전략을 동적으로 결정하는 '계획 기반 지각(planned perception)' 패러다임을 제시한다. 이는 [[concepts/embodied-ai.md|embodied ai]]의 능동 지각(active perception) 개념을 비디오 이해 영역으로 확장한 것이다.
