...
기존 Wiki의 [[concepts/retrieval-augmented-generation.md|retrieval augmented generation]] 개념과 직접적으로 연결되며, RAG에서 '검색 품질'이라는 근본 문제를 청킹 관점에서 조명한다. 또한 [[A Systematic Study of Retrieval Pipeline Design for RAG]] 논문과 상호 보완적 관계에 있다—해당 논문이 검색 파이프라인의 전체 설계를 다루었다면, 본 논문은 청킹이라는 특정 단계에 집중한다. 도메인 특화 지식 처리 측면에서 [[concepts/knowledge-injection.md|knowledge injection]] 개념과도 관련이 있으며, 기업 문서 처리라는 실용적 맥락에서 LLM 활용의 실질적 과제를 부각시킨다.
