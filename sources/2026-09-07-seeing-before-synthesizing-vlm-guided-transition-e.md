...
### 기존 Wiki 개념과의 관계
- [[concepts/blind-tool-invocation.md|blind tool invocation]]: 시각 검증 없이 합성 캡션을 훈련에 투입하는 것은 blind invocation의 파이프라인 버전으로, '사용 전 검증' 원칙이 도구 호출을 넘어 훈련 데이터 생성으로 확장된다.
- [[concepts/circular-validity-problem.md|circular validity problem]]: 합성 데이터가 감독 신호가 되면 타당성이 생성 가설에만 의존하는 순환 구조가 생기며, SBS는 VLM 관찰이라는 외부 근거 주입으로 이 루프를 절단한다.
- [[concepts/fixed-assignment-fragility.md|fixed assignment fragility]]: 고정 갭 할당의 구조적 취약성을 명시하고 그 대안 설계의 동기를 제공한다.

### 새 축
- [[concepts/adaptive-transition-discovery.md|adaptive transition discovery]], [[concepts/vision-grounded-synthesis.md|vision grounded synthesis]], [[concepts/synthetic-grounding-gap.md|synthetic grounding gap]]
