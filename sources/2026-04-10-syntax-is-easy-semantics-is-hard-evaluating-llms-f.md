...
주요 발견은 제목이 함축하듯, LLM이 LTL의 **구문(syntax)**은 비교적 잘 생성하지만 **의미론(semantics)**에서 체계적 오류를 보인다는 점이다. 이는 LLM의 형식 언어 처리 한계를 드러내며, [[concepts/formal-verification.md|formal verification]]과 [[concepts/temporal-logic.md|temporal logic]] 분야에서 LLM 활용 시 검증 파이프라인이 필수적임을 시사한다.

기존 Wiki의 [[concepts/code-generation.md|code generation]] 개념과 밀접하게 연결되는데, 자연어→형식 언어 변환은 코드 생성의 특수한 형태이다. 또한 [[concepts/llm-benchmark.md|llm benchmark]] 관점에서 형식 논리 영역의 새로운 평가 차원을 제공한다. [[concepts/reasoning-integrity.md|reasoning integrity]]와도 관련이 깊은데, LTL 의미론 오류는 LLM의 논리적 추론 신뢰성 문제를 반영한다. [[concepts/ai-safety.md|ai safety]] 측면에서 보안/프라이버시 정책 명세의 정확성은 안전성에 직결되므로, LLM 생성 LTL 수식의 의미론적 검증은 필수적이다.
