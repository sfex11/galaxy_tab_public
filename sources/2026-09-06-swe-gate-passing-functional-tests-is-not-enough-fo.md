...
- [[concepts/coverage-functionality-gap.md|coverage functionality gap]]의 확장: RESTestBench가 '커버리지가 기능적 검증 적합성을 측정하지 못한다'는 간극을 제시했다면, SWE-Gate는 '테스트 통과가 수용 가능성을 측정하지 못한다'는 동형 논리를 패치 평가로 확장한다.
- [[concepts/benchmark-specification-gap.md|benchmark specification gap]]의 구체 사례: 평가 단위(기능 테스트)와 배포 수용 단위(리뷰된 패치)의 불일치가 [[concepts/evaluation-deployment-unit-mismatch.md|evaluation deployment unit mismatch]]를 형성한다.
- [[concepts/evaluator-assumption.md|evaluator assumption]] 폭로: '테스트 통과 = 수용'이라는 암묵적 가정이 점수와 실세계 유용성을 단절시킨다.
- [[entities/swe-chat.md|swe chat]]과의 보완: SWE-chat이 야생 상호작용에서 태스크-현실 간극([[concepts/task-reality-divergence.md|task reality divergence]])을 보였다면, SWE-Gate는 평가 계약 자체가 수용 워크플로우를 누락함을 보인다.

## 핵심 인사이트

인간 개발자의 패치가 테스트를 통과해도 리뷰에서 거절되듯, 에이전트 패치도 리뷰 문화·아키텍처 관례·유지보수성 요구라는 암묵적 계약을 충족해야 한다. 이 계약은 코드에 존재하지 않는 [[concepts/repository-operational-knowledge.md|repository operational knowledge]]이며, 이를 측정하지 않는 벤치마크 점수는 실세계 기여도와 무관한 지표로 전락한다.
