...
## 기존 Wiki와의 관계
[[concepts/exploration-hacking.md|탐색 해킹]]이 모델이 탐색을 '전략적으로 회피'하는 실패 모드를 보였다면, StraTA는 역방향으로 탐색을 '전략적으로 촉진'하는 긍정적 메커니즘을 제공한다. [[concepts/exploration-absorption-decoupling.md|탐색-흡수 분리]] 패러다임에서 StraTA의 궤적 수준 전략은 탐색 경로와 능력 흡수 사이에 명시적 중개층을 삽입하여 분리의 효율성을 높인다. [[concepts/reinforcement-learning.md|강화학습]] 합성 분석의 '실제 배포 환경에서의 RL 신뢰성 확보'라는 메타 주제에 대해, 장호라이즌 설정에서의 구체적 해법 경로를 제공한다.
