...
이 논문은 Wiki가 축적한 핵심 패턴의 보안 도메인 구현이다. 첫째, [[harness-side-compensation]]의 가장 명시적 실현 — 모델이 처리 불가능한 위상적 의존성을 하네스(이종 그래프)가 완전히 흡수하여 '보정'이 아닌 '위임' 수준의 하네스 역할을 제시한다. 둘째, [[constraint-guided-plan-execution]]과 동형 — RunAgent가 계획 실행을 제약으로 강제했다면, 본 논문은 행동의 위상 일관성 자체를 구조가 보장한다. 셋째, [[agentic-security-investigation]]과 [[telecom-rca]]가 진단 추론(관찰→가설→인과 귀인→검증)의 도메인 불변성을 보였다면, 본 논문은 그 안에서 위상적 하위 문제를 인지적 하위 문제와 분리해 전담 구성요소에 위임하는 새로운 분해 축을 제공한다.

[[long-context]] 관점의 통찰: 그래프형 운영 데이터는 컨텍스트 압축·계층화(LongSeeker, ShallowStream)보다 외부 구조 위임이 본질적으로 적합하다 — 정보를 줄이는 것이 아니라 표현 계층 자체를 분리하는 것이다. 또한 [[reasoning-integrity]]의 보안 도메인 구체화로서 '권장 행동-토폴로지 일관성'이라는 보장 대상을 명시하고, [[structural-certification]]이 사후 인증이 아닌 아키텍처 내장 속성으로 실현되는 경로를 보여준다.
