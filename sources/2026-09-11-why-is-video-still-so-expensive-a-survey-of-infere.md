...
**기존 Wiki와의 관계**: 본 서베이는 비디오 도메인에 흩어져 있던 효율화 연구들([[concepts/token-pruning.md|token pruning]], [[concepts/text-guided-vision-token-selection.md|text guided vision token selection]], [[concepts/vision-token-kv-cache-redundancy.md|vision token kv cache redundancy]], [[concepts/inter-position-redundancy.md|inter position redundancy]])을 하나의 추론 효율 스펙트럼으로 통합하는 좌표를 제공한다. [[concepts/shallow-index-deep-answer.md|shallow index deep answer]](ShallowStream)의 지연 실행이 이 스펙트럼의 스트리밍 극점에 위치함을 확인하고, [[concepts/streaming-adaptive-inference.md|streaming adaptive inference]]와 [[concepts/adaptive-inference.md|adaptive inference]]의 적응 축을 프레임 수·컨텍스트 길이·태스크 복잡도로 확장한다.

**핵심 인사이트**: (1) 비디오의 비용 문제는 모델 크기가 아닌 입력 스케일링 문제다 — 프레임 수가 곧 컨텍스트 길이이므로 frame-count-cost-scaling이라는 독립 축이 필요하다. (2) temporal redundancy(인접 프레임 간 중복)가 효율화의 핵심 기회이며, 토큰 프루닝과 KV 캐시 재사용은 모두 이 중복성의 활용이다. (3) [[concepts/perception-cognitive-capacity-mismatch.md|perception cognitive capacity mismatch]]의 효율성 측 재해석: 프레임 수 증가는 관찰 충실도와 비용을 동시에 올리되 성능은 역설적으로 저하시킬 수 있어, '적게 보기'가 성능·비용 양축의 해법이 된다. (4) [[concepts/on-device-inference.md|on device inference]] 제약이 효율화 연구의 직접적 동기다.

**연결점**: 비디오는 [[concepts/long-context.md|long context]] 스케일링의 최대 부하 도메인이며, [[concepts/modality-asymmetric-memory-cost.md|modality asymmetric memory cost]](시각 토큰의 KV 비용 지배)가 극대화되는 곳이다. [[concepts/model-compression.md|model compression]]·[[concepts/speculative-decoding.md|speculative decoding]] 등 디코딩 가속은 아키텍처 최적화와 상보적 축을 형성한다.
...
- → [[sources/2026-09-11-beyond-one-size-fits-all-sample-adaptive-strategy-]]: 비디오 LLM 효율화 서베이가 토큰 프루닝·적응적 추론을 핵심 메커니즘으로 제시하는 가운데, 본 논문은 이를 샘플 적응적 전략 라우팅이라는 메타 계층으로 구체화한 사례다.
