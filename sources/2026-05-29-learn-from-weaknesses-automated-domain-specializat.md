...
- → [[sources/2026-05-28-muse-autoskill-self-evolving-agents-via-skill-crea]]: 에이전트의 도메인 특화 및 스킬 획득 문제를 다루며, 전자는 소형 에이전트의 약점을 기반으로 하네스 데이터를 자동 생성해 학습시키고, 후자는 스킬의 생성, 기억, 평가를 통한 자가 진화 메커니즘을 제안합니다.
- → [[sources/2026-05-29-swarmharness-skill-based-task-routing-via-decentra]]: 모두 '스킬'을 중심으로 에이전트의 작업을 특화 및 라우팅하는데, 전자는 분산형 인센티브 정렬 네트워크를 통한 에이전트 간 스킬 기반 라우팅을, 후자는 개별 소형 에이전트 내의 도메인별 스킬 소비 간극 해소를 다룹니다.
