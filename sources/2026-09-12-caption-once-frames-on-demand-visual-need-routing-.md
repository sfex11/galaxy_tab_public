...
1. [[concepts/shallow-index-deep-answer.md|shallow index deep answer]]의 모달리티 확장 — ShallowStream이 지연 실행을 인덱스 계층에 적용했다면, CFD는 이를 시각 증거 계층으로 이동시켜 '캡션 1회→프레임 온디맨드'라는 지연 관찰 패턴을 수립한다. Seeing Before Synthesizing이 관찰 선행 원칙(훈련 데이터 축)이라면 CFD는 추론 시점 지연 관찰(예산 축)로 상보적 위치를 형성한다.

2. [[concepts/visual-memory-textual-collapse.md|visual memory textual collapse]]에 대한 구조적 응답 — 텍스트 전용 메모리의 시각 속성 소실을 인정하되 망각 보상을 온디맨드 재관찰로 대체한다. 시각 필요성 라우팅이 붕괴 비용을 질의 유형별로 관리 가능하게 만든다.

3. [[entities/video-inference-efficiency.md|video inference efficiency]] 서베이(2026-09-11)에 새 축 추가 — 시각 토큰 전면 처리 대신 증거가 필요한 순간만 검색하는 것이 장시간 비디오의 지배적 비용 절감 축임을 보여준다.

4. [[concepts/lazy-schema-loading.md|lazy schema loading]]과 동형 구조 — 도구 스키마가 아닌 프레임을 지연 로딩함으로써 '불필요한 컨텍스트 원천 차단' 원리가 도구 계층에서 지각 계층으로 확장된다.

5. [[concepts/modality-asymmetric-memory-cost.md|modality asymmetric memory cost]]를 기능적 우위 비대칭으로 보완 — 비용 비대칭에 기능 비대칭(시간 구조 vs 속성 지각)을 결합한 이중 구조를 제시한다.

라우팅 결정은 [[concepts/expected-value-of-information.md|expected value of information]]의 구현이며 [[concepts/adaptive-inference.md|adaptive inference]]의 축을 '얼마나 계산할까'에서 '언제 볼까'로 확장한다. [[concepts/strategy-routing.md|strategy routing]]과 대비해 라우팅 변수가 샘플 특성이 아닌 질의의 인식 요구임을 보인다. 엣지 제약 하에서 [[concepts/on-device-inference.md|on device inference]] 프론티어를 이동시키고, [[concepts/caption-sufficient-evaluation.md|caption sufficient evaluation]]의 역문제(언제 캡션이 불충분한가)를 런타임 판별 문제로 전환한다.
