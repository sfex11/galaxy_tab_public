...
Wiki 축적 지형과의 관계: (1) [[multi-signal-hallucination-detection]](Domain-Specific Hallucination Detection)이 분류기·불확실성·보정 3신호로 '생성 완료 후' 환각을 감지했다면, 본 논문은 개입 시점을 '생성 중'으로 이동시켜 환각 개입의 사후 감지—선제 억제 이중 구조를 완성한다. (2) [[unrecoverable-reasoning-error]]이 규정한 '초기 오류의 자기회귀적 복합화로 사후 복구 불가' 실패 모드에 대한 최초의 디코딩 수준 처방이 된다. (3) 슬라이딩 윈도우 MLP 어블레이션은 [[causal-mechanistic-interpretability]]의 개입 실험을 해석 도구에서 디코딩 제어기의 근거 신호로 전환한다. (4) [[intra-generative-intervention]]의 '토큰 생성 중 실시간 개입' 패러다임을 사실 정확성 도메인에서 구현하며, [[residual-stream-monitoring]]과 [[inference-time-behavior-control]]이 개척한 내부 신호 기반 추론 시점 개입 계열에 환각 억제라는 새 응용 축을 추가한다.
