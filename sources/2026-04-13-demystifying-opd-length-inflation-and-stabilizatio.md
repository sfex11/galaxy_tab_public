...
이 논문은 기존 Wiki의 [[concepts/on-policy-distillation.md|on policy distillation]] 개념과 직접적으로 연결되며, 특히 Nemotron-Cascade 2에서 다룬 cascade RL 및 distillation 파이프라인의 안정성 문제를 심화한다. [[concepts/truncation-collapse.md|truncation collapse]]와 [[concepts/length-inflation.md|length inflation]] 개념은 이미 2026-04-12자로 등록되어 있으며, 본 논문이 해당 개념의 주요 출처이다.

[[concepts/scaling-laws.md|scaling laws]] 및 [[concepts/curriculum-learning.md|curriculum learning]]과도 관련이 있는데, OPD의 길이 팽창은 훈련 데이터 분포가 학습 중 동적으로 변하는 non-stationary 문제이기 때문이다. [[concepts/reasoning-shift.md|reasoning shift]]에서 다룬 컨텍스트에 따른 추론 길이 변화 현상과도 맥락을 공유한다.
...
- → [[sources/2026-04-14-from-reasoning-to-agentic-credit-assignment-in-rei]]: 둘 다 LLM 강화학습의 훈련 안정성 문제를 다루며, 전자는 희소 보상 하의 크레딧 할당 문제를, 후자는 on-policy distillation의 길이 팽창 문제를 분석한다.
